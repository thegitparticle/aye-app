...
function CraftGif(props) {
  ...
  const transparent_overlay_view = useStyle(
    () => [
      {
        width: windowWidth,
        height: undefined,
  ...
  return (
    <View style={overall_view_wrap}>
      ...
      />
      <FastImage style={image_style} source={{uri: props.gifPicked}}>
        <ViewShot
          ref={viewShotGIFPickerRef}
          options={{format: 'png', quality: 0.9}}
          // eslint-disable-next-line react-native/no-inline-styles
          style={{backgroundColor: 'transparent'}}>
          <View style={transparent_overlay_view}>
            <Draggable
              children={Children()}
              x={0}
              y={windowWidth * 0.7}
...
export default connect(mapStateToProps)(CraftGif);
